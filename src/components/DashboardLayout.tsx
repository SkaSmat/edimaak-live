      ...
      <div className="min-h-screen flex w-full bg-background relative">
        ...
        <SidebarInset className="flex-1 w-full flex flex-col min-h-screen overflow-x-hidden">
          <div className="md:hidden sticky top-0 z-30 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b border-border/40">
            <DashboardMobileHeader fullName={fullName} onLogout={handleInternalLogout} unreadCount={unreadCount + pendingMatchCount} />
          </div>
          ...
          <header className="hidden md:flex items-center justify-between px-6 py-4 bg-card/50 border-b border-border/30 sticky top-0 z-10 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-semibold text-foreground pr-0 pl-[60px] pb-0">Bonjour, {firstName}</h1>
            ...
            variant="outline" size="sm" onClick={switchRole} className="gap-2 border-primary text-primary hover:bg-primary hover:text-white transition-all shadow-sm" title={`Cliquez pour passer au mode ${role === "traveler" ? "Expéditeur" : "Voyageur"}`}>
                  <RefreshCw className="h-4 w-4" />
                  Passer au mode {role === "traveler" ? "Expéditeur" : "Voyageur"}
                </Button>}

              <Button variant="ghost" size="sm" onClick={handleInternalLogout} className="text-muted-foreground hover:text-destructive transition-colors" title="Se déconnecter">
